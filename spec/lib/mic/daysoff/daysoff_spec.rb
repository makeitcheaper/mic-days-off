...
RSpec.describe Mic::Daysoff do
  ...
  describe '#is_today_day_off?' do
    ...
    context 'online file available' do
      before do
        stub_request(:get, "https://s3-eu-west-1.amazonaws.com/static-settings/days-off/staging/days-off.yml").
          with(headers: {
         	  'Accept'=>'*/*',
  ...
  describe '#_get_dates_from_web' do
    ...
    #     # allow(ENV).to receive(:[]).with("MIC_DAYS_OFF_YML_URL_FILE").and_return(stub_s3_file_url)
    #     stub_request(:get, stub_s3_file_url).
    #       with(headers: {
    #         'Accept'=>'*/*',
    #         'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
    #         'User-Agent'=>'Ruby'
